...
{  
   ...
   //update height of tree  
    ...
    if (tree->key_value > val) {
      tree->left = insert(tree->left, val); //tree->left = that?
    }

    else if (tree->key_value < val) {
      tree->right = insert(tree->right, val); //tree->right = that?
    }
    ...
    int balance = get_balance(tree);//HEIGHT(tree->left) - HEIGHT(tree->right);
